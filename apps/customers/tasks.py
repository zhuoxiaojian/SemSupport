...
from utils.getNeedDatas import get_sale, get_seo_sale, get_normal_sale
...
def divide_the_work():
    ...
    count = int(get_count())
    print('---------------------开始分配销售人员任务----------------------------')
    divide_the_customer_work(get_sale_info_to_task(), count)
    print('---------------------销售人员任务分配完毕----------------------------')
    print('---------------------开始分配普通销售任务----------------------------')
    divide_the_customer_work(get_normal_sale_info_to_task(), count)
    print('---------------------普通销售任务分配完毕----------------------------')
...
def get_normal_sale_info_to_task():
    date_from = getConstantsVale('dateForm')
    if date_from is None:
        date_from = '2017-01-01 00:00:00'
    date_to = get_today()
    ...
    normal_sale = get_normal_sale()
    return normal_sale, date_from, date_to, date_from_two, date_to_two
    ...
    if frcDatas.exists():
        for frcData in frcDatas:
            if frcData.name in handleCity:
                work_two(frcData, saleUser, date_from, date_to, date_from_two, date_to_two, SalecountNum_two, count)
            elif frcData.name == '远程':
                work_three(filterCity, saleUser, date_from, date_to, date_from_two, date_to_two, SalecountNum_three,
                           count)
            else:
                work_one(frcData, saleUser, date_from, date_to, date_from_two, date_to_two, SalecountNum, count)
                ...
                for sem in sem_sale:
                    ...
                    if len(sem_sale_id) > 0:
                        ...
                        q = FormCustomer.objects.filter(Q(sem_status=0),
                                                        Q(aike_status=0),
                                                        Q(create_time__range=(date_from, date_to)) | Q(
